...
class DispNetS(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(len(x.data)):
            ...
            p = disp1[i].data.cpu().numpy()
            print(gray.shape)
            print(p.shape)
            print(type(gray[0][1][1]))
            result = guidedFilter(gray, p)
            disp1[i].data = torch.FloatTensor(result).cuda()
        #    print(gray.shape)
...
def guidedFilter(I, p, sz=60, eps=1E-6):
    #print(I.shape)
    #print(p.shape)
    img_I   = I.astype(np.float64)
    img_p   = p.astype(np.float64)

    fmean   = lambda img: cv2.boxFilter(img, cv2.CV_64F, (sz, sz))


    mean_I  = fmean(img_I)
    mean_p  = fmean(img_p)
    corr_I  = fmean(img_I * img_I)
    corr_Ip = fmean(img_I * img_p)


    var_I   = corr_I  - mean_I * mean_I
    conv_Ip = corr_Ip - mean_I * mean_p
